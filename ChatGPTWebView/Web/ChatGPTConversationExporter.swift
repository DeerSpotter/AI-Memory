...
enum ChatConversationExportError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .invalidPayload: return "The ChatGPT page did not return a readable export payload."
        case .noMessagesFound: return "No ChatGPT messages were found. Open a conversation and try again."
...
final class ChatGPTConversationExporter {
    static func exportConversation(from webView: WKWebView) async throws -> ChatConversationExportResult {
        let raw = try await evaluateJavaScript(extractionJavaScript, in: webView)
        guard let json = raw as? String, let data = json.data(using: .utf8) else { throw ChatConversationExportError.invalidPayload }
        ...
        guard payload.messageCount > 0, !markdown.isEmpty else { throw ChatConversationExportError.noMessagesFound }
        let pdfData = try makePDFData(title: payload.title, markdown: markdown, sourceURL: payload.sourceURL, exportedAt: payload.exportedAt, messageCount: payload.messageCount)
        return ChatConversationExportResult(title: payload.title, markdown: markdown, messageCount: payload.messageCount, sourceURL: payload.sourceURL, exportedAt: payload.exportedAt, pdfData: pdfData)
    }
    ...
    private static func evaluateJavaScript(_ script: String, in webView: WKWebView) async throws -> Any {
        try await withCheckedThrowingContinuation { continuation in
            webView.evaluateJavaScript(script) { result, error in
                ...
                else { continuation.resume(throwing: ChatConversationExportError.invalidPayload) }
            }
        }
    }

    private static func makePDFData(title: String, markdown: String, sourceURL: String, exportedAt: String, messageCount: Int) throws -> Data {
        let html = """
        ...
        guard renderer.numberOfPages > 0 else { throw ChatConversationExportError.cannotCreatePDF }
    ...
    (() => {
      ...
      const title = (() => {
        for (const selector of ['h1:not([class*=hidden])','[data-testid*=conversation-title]','[class*=conversation-title]']) {
          const value = normalize(document.querySelector(selector)?.textContent || '');
          if (value && !/^(chatgpt|new chat|untitled|chat)$/i.test(value)) return value;
        }
        const doc = normalize(document.title || '').replace(/\s*[-|]\s*ChatGPT\s*$/i,'');
        return doc && !/^(chatgpt|new chat|untitled|chat)$/i.test(doc) ? doc : 'ChatGPT Conversation';
      })();
      ...
      const findMessages = () => {
        for (const selector of ['[data-message-author-role]','article[data-testid*=conversation-turn]','div[data-testid*=conversation-turn]','.group\\/conversation-turn','[data-message-id]']) {
      ...
      const header = [`# ${title}`,'',`**Exported:** ${exportedAt}`,`**Source:** ${window.location.href || 'chatgpt.com'}`,`**Messages:** ${messages.length}`,'','---',''];
      return JSON.stringify({ title, markdown: header.concat(body).join('\n').trim() + '\n', messageCount: messages.length, sourceURL: window.location.href || '', exportedAt });
    })();
    """#
}
